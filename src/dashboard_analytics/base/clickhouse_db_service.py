...
class ServiceBase:
    ...
    def find_all_ab(self, findManyOptions, offset=None, limit=None,):

        try:
            if offset is not None:
                results = self.session.query(self.model).filter(findManyOptions).offset(offset).limit(limit).all()
            else:
                results = self.session.query(self.model).filter(findManyOptions).all()
            return results
        except exc.SQLAlchemyError as e:
            raise DbException(e)
        except Exception as e:
    ...
    def find_many_ab(self, findManyOptions, sort_by_key=None, offset = None, limit=None, fields="*"):
        from sqlalchemy.orm import load_only
